...
def test_create_weather():
    response = client.post( "/weather", json={"location": "UA", "temperature": 25} )
...
def test_update_weather():
    new_id = client.post("/weather", json={"location": "UA", "temperature": 25}).json()
    response = client.put(f"/weather?id={new_id}", json={"location": "UAE", "temperature": 26})
    assert response.status_code == 200
    assert response.json() =={"status": "temperature was updated"}